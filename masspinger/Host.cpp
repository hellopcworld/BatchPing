...
Host::Host(boost::asio::io_service& io_service, const std::string& host)
    : //logger(logger),
      sequence_number(0),
      is_responsive(false),
...
void Host::set_responsive()
{
    if (is_responsive != true)
    {
        ...
        {
            //cout << host_ip_address << " is alive" <<endl;
            m_IsAlive = 1;
...
{
    if ((is_responsive != true) && (error != boost::asio::error::operation_aborted))
    {
        if (is_hostname_available)
        {
        ...
        is_responsive = false;
        m_IsAlive = 0;
        //scout << host_ip_address << "  is not alive" <<endl;
